...
def loss(pred_xyz, pred_r, pred_mask, pred_score, gt_xyz, gt_r, global_step, batch_size, dim=3, h=240, w=320):
  ...
  gt_scores = tf.exp(-tf.reduce_sum(tf.squared_difference(pred_xyz, gt_xyz),3)) 
  gt_scores = tf.reshape(gt_scores,[-1,h,w,1])

  loss_score = tf.reduce_mean( tf.squared_difference(pred_mask, gt_scores))
  loss_elem = tf.reduce_mean(tf.squared_difference(pred_xyz,gt_xyz))
 
  loss_boundary = tf.reduce_sum(tf.squared_difference(gt_r, pred_r))/tf.reduce_sum(obj_mask_origin + 0.000001)
  loss_variance = 0.0
  loss_violation = 0.0

  loss_score = tf.reduce_mean(tf.squared_difference(pred_xyz,gt_xyz))
