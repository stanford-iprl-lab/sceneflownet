...
def loss(pred_feat, pred_r, pred_mask, pred_score, gt_feat, gt_r, gt_score, batch_size, dim=9, h=240, w=320):
  obj_mask_origin = tf.greater(gt_feat[:,:,:,2],tf.zeros_like(gt_feat[:,:,:,2]))
  ...
  loss_elem = tf.reduce_sum(tf.squared_difference(pred_feat, gt_feat)*obj_mask_dim)/tf.reduce_sum(obj_mask_1 + 0.000001)
  ...
  for b_i in xrange(batch_size):
    ...
    ones = tf.ones_like(gt_feat[b_i,:,:,dim-1])
    
    obj_mask = obj_mask_origin[b_i]

    def instance_variance_loss(z):
      idx_mask = tf.equal(gt_feat[b_i,:,:,2], ones * z)
      idx_mask = tf.reshape(idx_mask, [h,w,1])
      idx_mask = tf.cast(idx_mask, tf.float32)
      idx_mask_dim  = tf.tile(idx_mask, [1,1,dim])
      tmp_prd = idx_mask_dim * pred_feat[b_i]
      tmp_prd = tf.reshape(tmp_prd,(-1,dim))
      tmp_mean = tf.reduce_sum(tmp_prd,axis=0)/(tf.reduce_sum(idx_mask)+0.000001)
      tmp_mean = tf.reshape(tmp_mean,(1,1,dim))
      tmp_mean_final = tf.tile(tmp_mean,[h,w,1])
      loss_variance_instance = tf.reduce_sum(idx_mask_dim * tf.squared_difference(tmp_mean_final, pred_feat[b_i])) / (tf.reduce_sum(idx_mask)+0.000001)
    ...
    def instance_violation_loss(z):
      ...
      tmp_prd = idx_mask_dim * pred_feat[b_i,:,:,0:dim]
      tmp_prd = tf.reshape(tmp_prd,(-1,dim))

      tmp_r = idx_mask_dim[:,:,0:1] * pred_r[b_i]
      tmp_r_mean = tf.reduce_sum(tmp_r)/(tf.reduce_sum(idx_mask)+0.000001)
      r = tmp_r_mean * 0.5

      friend_mask = idx_mask_dim[:,:,0]
      l2_error = tf.reduce_sum(tf.squared_difference(pred_feat[b_i], gt_feat[b_i]),2)
    ...
    return loss_mask, loss_score, loss_elem, loss_variance, loss_boundary, loss_violation     
