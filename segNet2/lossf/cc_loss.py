...
def loss(pred_cc,gt_cc_mask, gt_cc):
  loss_cc = tf.reduce_mean(tf.squared_difference(pred_cc,gt_cc))
  return loss_cc

def loss_(frame2_input_xyz, gt_frame1_pred_xyz, pred_xyz, pred_r, pred_mask, pred_score, pred_transl,pred_rot,pred_cc,gt_cc_mask, gt_cc, gt_rot, gt_transl, gt_xyz, gt_r, gt_score, batch_size, dim=3, h=240, w=320): 
