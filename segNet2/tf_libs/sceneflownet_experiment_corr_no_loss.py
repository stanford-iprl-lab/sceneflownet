...
class Experiment:
    ...
    try:
      ...
      while not coord.should_stop():
        _,self.lossv['total_loss'],\
            ...
            self.loss['score'],\
            self.loss['flow']])
        self.loss_value_add({'transl':self.lossv['transl'],'quater':self.lossv['quater'],'flow':self.lossv['flow'],'boundary':self.lossv['boundary'],'total_loss':self.lossv['total_loss'],'mask':self.lossv['mask'], 'score':self.lossv['score'], 'elem3':self.lossv['elem3'], 'elem6':self.lossv['elem6']})
        step += 1
  ...
  def validate_epoch(self,restore_epoch=0):
    ...
    for ii in xrange(self.num_batch):
        self.lossv['total_loss'],\
            ...
            self.loss['score'],\
            self.loss['flow']])
        self.loss_value_add({'quater':self.lossv['quater'],'transl':self.lossv['transl'],'flow':self.lossv['flow'],'boundary':self.lossv['boundary'],'total_loss':self.lossv['total_loss'],'mask':self.lossv['mask'], 'score':self.lossv['score'], 'elem3':self.lossv['elem3'],'elem6':self.lossv['elem6']})
  ...
  def result_op(self,save=True):  
    if 0:
      ...
      #self.predv['instance_center'], self.predv['instance_boundary'], self.predv['instance_score'] = nms(self.predv['traj_masked'][0], self.predv['frame2_boundary_masked'][0][:,:,0], self.predv['frame2_score_positive_masked'][0])
      #self.predv['final_seg']= infer_seg(self.predv['instance_center'],self.predv['instance_boundary'],self.predv['instance_score'],self.predv['traj_masked'][0])
      #print(self.predv['final_score']) 
    else:
      ...
      for i in range(self.batch_size):
        ...
        if save:
          ...
          gt_score_path = os.path.join(tmp_path,'gt_score.npz')
          pred_score_path = os.path.join(tmp_path,'pred_score.npz')
          np.savez(gt_score_path,score=self.predv['frame2_score_positive_masked'][i])
          np.savez(pred_score_path,score=self.gtv['frame2_score'][i])
          pred_r_path = os.path.join(tmp_path,'pred_r.npz')
          np.savez(pred_r_path,r=self.predv['frame2_boundary_masked'][i])
          gt_r_path = os.path.join(tmp_path,'gt_r.npz')
          np.savez(gt_r_path,r=self.gtv['frame2_r'][i])
  ...
  def whole_process(self):
    ...
    #best_epoch = self.validate(0,80)#self.flags.num_epochs)
