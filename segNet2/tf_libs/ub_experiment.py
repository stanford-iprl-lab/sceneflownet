...
class Experiment:
  ...
  def build_model(self, tfrecords_filename, num_epochs):
    ...
    self.cc_error = tf.random_uniform(shape=[self.batch_size,30,40,441],maxval=1.0)
    self.cc_error = tf.nn.relu(tf.sign(self.cc_error-0.95)) * (self.cc_error - 0.05)
    self.gt['cc'] = self.gt['cc'] + self.cc_error
  ...
  def loss_op(self):
    ...
    self.cost = self.loss['flow'] * 100  + self.loss['transl'] * 200  +  self.loss['rot']#+ self.loss['mask'] # + self.loss['elem'] * 100.0 + self.loss['boundary'] * 1000.0 + self.loss['score'] * 0.5 + self.loss['violation'] + self.loss['variance']
