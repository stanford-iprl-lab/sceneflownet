...
class Experiment:
  ...
  def build_model(self, tfrecords_filename, num_epochs):
    ...
    #  self.pred['transl'], \
    #  self.pred['rot']\
    self.pred['cc']  = self.model(self.input['frame1_xyz'],self.input['frame1_rgb'],self.input['frame2_xyz'],self.input['frame2_rgb'])#,\
  ...
  def loss_op(self):
    ...
    self.loss['cc'] =  self.lossf(self.pred['cc'],self.gt['mask_cc'],self.gt['cc'])
    #self.loss['cc'],\
    #self.loss['flow'], \
    #self.loss['rot'], \
   ...
   #   self.pred['frame2_score'], \
    ...
    self.cost = self.loss['cc']# * 00.001 + self.loss['flow'] * 100  + self.loss['transl'] * 200  +  self.loss['rot']#+ self.loss['mask'] # + self.loss['elem'] * 100.0 + self.loss['boundary'] * 1000.0 + self.loss['score'] * 0.5 + self.loss['violation'] + self.loss['variance']
  ...
  def train(self,restore_epoch=-1):
    ...
    try:
      ...
      while not coord.should_stop():
        _, self.lossv['total_loss'], self.lossv['cc'] = self.sess.run([self.train_op,self.cost,self.loss['cc']])
        #self.lossv['flow'], \
        #self.lossv['rot'], \
        #self.lossv['transl'],\ 
...
#        self.loss_value_add({'transl_variance':self.lossv['transl_variance'],'transl':self.lossv['transl'],'total_loss':self.lossv['total_loss'],'mask':self.lossv['mask'],'elem':self.lossv['elem'],'boundary':self.lossv['boundary'], 'violation':self.lossv['violation'],'variance':self.lossv['variance'], 'score':self.lossv['score']}) 
        ...
        self.lossv['cc'] = self.sess.run(self.loss['cc'])
        self.loss_value_add({'cc':self.lossv['cc']})#, 'flow':self.lossv['flow'] * 100.0, 'total_loss':self.lossv['total_loss'],'rot':self.lossv['rot'],'transl':self.lossv['transl']*100.0})
